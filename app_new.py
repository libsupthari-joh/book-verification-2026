...
if current is None:
    ...
    if neon_df.empty:
        ...
        lib_filter_df = pd.DataFrame()
        if id_col:
            lib_filter_df = neon_df[neon_df[id_col].astype(str).str.strip().str.upper() == st.session_state["librarian_id"].strip().upper()]
        if lib_filter_df.empty and lib_name_col and st.session_state["librarian_location"]:
            lib_filter_df = neon_df[neon_df[lib_name_col].astype(str).str.upper().str.contains(st.session_state["librarian_location"].upper())]
        ...
        if current == "நூலகத் தரவுகள்":
            ...
            if not lib_filter_df.empty:
                ...
                st.info(f"ℹ️ உங்களது ஐடி ({st.session_state['librarian_id']}) தொடர்பான நூலகத் தரவுகள் எதுவும் டேட்டாபேஸில் காணப்படவில்லை.")
                
        elif current == "பதிவிறக்கம்":
            st.subheader(f"📥 நூலகத் தரவுகள் பதிவிறக்கம்")
            if not lib_filter_df.empty:
                st.dataframe(lib_filter_df, use_container_width=True)
                csv_data = lib_filter_df.to_csv(index=False).encode('utf-8-sig')
                st.download_button(
                    label="📥 உங்களது நூலகத் தரவுகளைப் பதிவிறக்குக (CSV)",
                    data=csv_data,
                    file_name=f"Library_Data_{st.session_state['librarian_id']}.csv",
                    mime="text/csv",
                    type="primary"
                )
            else:
    ...
    if current == "பிரிக்க":
        ...
        st.subheader("❌ தவறான பதிவினை நீக்குதல் / திருத்துதல்")
            ...
            if sel_pub != "-- பதிப்பகத்தைத் தேர்ந்தெடுக்கவும் --":
                ...
                if sel_title != "-- தலைப்பைத் தேர்ந்தெடுக்கவும் --":
                    ...
                    if target_item:
                        ...
                        st.markdown(f"**📖 தலைப்பு:** {sel_title} | **தேவை:** {req_q} | **பெறப்பட்டது:** {rec_q}")
                        new_received = st.number_input("புதிய பெறப்பட்ட எண்ணிக்கை:", min_value=0, max_value=500, value=rec_q)
                        
                        col_upd, col_del = st.columns(2)
                        with col_upd:
                            if st.button("💾 புதுப்பி (Update)", type="primary", use_container_width=True):
                                try:
                                    conn = psycopg2.connect(DB_URL)
                                    cur = conn.cursor()
                                    ...
                                    st.error(f"❌ பிழை: {e}")
                        with col_del:
                            if st.button("🗑️ நீக்கு (Delete)", type="secondary", use_container_width=True):
                                try:
                                    conn = psycopg2.connect(DB_URL)
                                    cur = conn.cursor()
                                    ...
                                    conn.close()
                                    st.session_state["submitted_reports"] = load_submitted_reports_from_db()
                                    st.success("✅ வெற்றிகரமாக நீக்கப்பட்டது!")
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"❌ பிழை: {e}")
        ...
        if st.session_state["submitted_reports"]:
            ...
            st.download_button("📥 முழுமையான தரவுகளை Excel கோப்பாகப் பதிவிறக்குக", data=csv_data, file_name=f"Master_Data_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv", type="primary")
        else:
            st.info("ℹ️ பதிவிறக்கம் செய்யத் தரவுகள் எதுவும் இல்லை.")
