                                ...
                                try:
                                    with st.spinner("சீட்டில் சேமிக்கப்படுகிறது..."):
                                        ...
                                        if rows_written < len(records_to_save):
                                            ...
                                            st.rerun()
                                    # NOTE: automatic Drive upload was removed on request — personal
                                    # (non-Workspace) Gmail accounts don't give service accounts the
                                    # storage quota needed to create files there, so it always failed
                                    # with a 403. The PDF still downloads locally via the button above,
                                    # and the Sheet write above is the source of truth.
                                    st.session_state["task1_save_message"] = (
                                        "success",
